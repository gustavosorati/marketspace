...
const createAdSchema = z.object({
  name: z.string(),
  description: z.string().max(140, "Limite de 140 caracteres"),
  price: z.string(),
})

type CreateAdSchemaType = z.infer<typeof createAdSchema>;

export function NewAd() {
  const theme = useTheme();
  const navigation = useNavigation<PrivateParamsTypes>();

  const { control, handleSubmit } = useForm<CreateAdSchemaType>({
  ...
  const handleSeePreview = async (data: CreateAdSchemaType) => {
    if(images.length === 0) {
      Alert.alert(
