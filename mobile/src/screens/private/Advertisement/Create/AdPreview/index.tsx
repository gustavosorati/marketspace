...
export function AdPreview() {
  ...
  const publishProductPhotos = async (productId: string) => {
    try {
      ...
      await PRODUCTIMAGES_SERVICES.create(formData);
  ...
  const handlePublishAd = async () => {
    try {
      setIsLoading(true);
      const product = await PRODUCTS_SERVICES.create({
        name: params.name,
      ...
      if(product?.id) {
        await publishProductPhotos(product.id);
      }
    } catch (error) {
      ToastNotification({
        type: "error",
